...
export function renderPlantCard(p, harvests) {
  ...
  var tlHtml = '<div class="stage-timeline" id="tl-' + p.id + '" style="' + (isOpen ? '' : 'display:none') + '">' +
    stageOrder.map(function(sk) {
      ...
      return '<div class="stage-row' + (isA ? ' active-stage' : '') + '">' +
        ...
        '<span class="stage-date">' + fmt(stages[sk]) + '</span>' + (isE ? '<span class="stage-edited">✏️</span>' : '') + '</div>';
    }).join('') + '</div>';
  ...
  return '<div class="plant-card stage-' + stage + '">' +
    ...
    (!p.is_harvested ? '<div style="font-size:10px;color:var(--text3)">✂️</div><div style="font-size:13px;font-weight:600">' + fmt(stages.harvest) + '</div>' + (warn ? '<div class="badge badge-' + warn.type + '" style="margin-top:3px;font-size:10px">' + warn.text + '</div>' : '') : '<span class="badge badge-done">✅</span>') +
    '</div></div>' +
    (!p.is_harvested ? '<div class="progress-wrap"><div class="progress-bar ' + barClass + '" style="width:' + pct + '%"></div></div><div style="font-size:11px;color:var(--text3);text-align:right;margin-bottom:4px">' + pct + '% ' + t('pct') + '</div>' : '') +
...
export function renderPlantList(p, harvests) {
  ...
  var html = '<div class="plant-list-item stage-' + stage + '" onclick="GrowLog.toggleExpand(\'' + p.id + '\')">' +
    ...
    '<div style="font-size:11px;color:var(--text3)">' + v.name + ' · ✂️ ' + fmt(stages.harvest) + (warn ? ' ⚠️' : '') + (p.is_harvested && ph.length ? ' ✅ ' + totalW.toFixed(1) + 'g' : '') + '</div></div>' +
...
export async function openEditPlant(plantId) {
  ...
  try {
    ...
    if (variety) { setType(variety.seed_type || 'auto'); document.getElementById('var-min').value = variety.flowering_days_min || ''; document.getElementById('var-max').value = variety.flowering_days_max || ''; }
    openModal('modal-plant');
...
export async function savePlant() {
  ...
  if (!date) errs.push(t('errDate')); if (!varName) errs.push(t('errVariety'));
  if (varType === 'auto') { if (!minV) errs.push(t('errMin')); if (!maxV) errs.push(t('errMax')); if (minV && maxV && min > max) errs.push(t('errMinMax')); }
...
export function openStageModal(plantId, labelEnc) {
