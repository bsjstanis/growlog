...
export async function renderFinance() {
  var pg = document.getElementById('page-finance'); pg.innerHTML = '<div class="spinner"></div>';
  try {
    ...
    var allExpenses = await sb('expenses', 'GET', null, '?order=created_at.desc');
    ...
    var allYears = [...new Set([].concat(
      ...
      allHarvests.map(function(h) { return getYear(h.harvest_date); }),
      allPlants.map(function(p) { return getYear(p.plant_date); }),
      [curYear]
    ...
    var yPlants = allPlants.filter(function(p) { return getYear(p.plant_date) === state.financeYear; });
    var yHarv = allHarvests.filter(function(h) { return getYear(h.harvest_date) === state.financeYear; });
    ...
    var html = '<div class="year-tabs">' + allYears.map(function(y) {
...
export async function saveExpense() {
  ...
  try {
    await sb('expenses', 'POST', { year: state.financeYear, category: document.getElementById('exp-category').value, amount_uah: amt, notes: document.getElementById('exp-notes').value.trim() });
